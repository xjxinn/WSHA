def daily(Day, TimeBegin, TimeEnd, HStoinitVal, AStoinitVal, HStofinalVal, AStofinalVal, NH3Disp, CapaWind, CapaSolar,
          CapaHE, CapaASR, CapaHB, CapaAB):
    import pandas as pd
    import math
    import cvxpy as cp
    import numpy as np
    ...
    #  现货交易决策
    pwindToNorm = pd.read_excel('RESData/ResPredpu_15min.xlsx', header=None)
    SpotPrice = pd.read_excel('RESData/ElecPrice_15min.xlsx', header=None)
    psolarToNorm = 0
    VirtualNH3Demand = pd.read_excel('VirtualNH3Demand_15min.xlsx', index_col=0)
    ActualNH3Demand = pd.read_excel('ActualNH3Demand_15min.xlsx', index_col=0)
    ##
    # 模型参数
    ...
    # 定容优化模型
    horizonDispatch = TimeEnd - TimeBegin  # 规划的时间区间
    ...
    ## 目标函数
    # 年运行收益：上网收益
    fFuncProfit = np.array(SpotPriceSell.iloc[TimeBegin:TimeBegin + nDispatch]).T @ varsCtrlDisp[:, 0]
    # 年运行成本：购买网电成本
    fFuncEleCost = np.array(SpotPrice.iloc[TimeBegin:TimeBegin + nDispatch]).T @ varsSpotMarket[:, 0]
    fFuncCost = fFuncEleCost
    # 日末SOC惩罚项
    punish = cp.abs(varsSto[nDispatch, 1] - AStofinalVal)

    # 年净运行成本
    fFuncDisp = fFuncCost - fFuncProfit + punish
    ...
    for itr in range(nDispatch):
        # 功率平衡约束
        constraint += [(RES[Day * 24 * 4 + TimeBegin + itr] + varsCtrlDisp[itr, 1]) - (
                    varsCtrlDisp[itr, 0] + varsCtrlDisp[itr, 2] + varsCtrlDisp[itr, 3]) >= 0]
        # 储氢罐约束
        constraint += [
            varsSto[itr + 1, 0] - varsSto[itr, 0] - (varsCtrlDisp[itr, 4] - varsCtrlDisp[itr, 5]) / 4 == 0]  # 状态空间方程
        ...
        # 储氨罐约束
        constraint += [
            varsSto[itr + 1, 1] - varsSto[itr, 1] + varsAmmoniaMarket[itr, 0] / 4 - varsCtrlDisp[itr, 3] / 4 / (
                3.230e-4) * ConstG2L == 0]  # 状态空间方程
    ...
    # 合成氨工况切换约束
    constraint += [varsCtrlDisp[:, 3] - NH3Disp == 0]

    # 储氢罐初值和终值约束
    constraint += [varsSto[0, 0] - HStoinitVal == 0,
                   varsSto[nDispatch, 0] - HStofinalVal == 0,
                   varsSto[nDispatch, 0] - yitaMin * CapaHB >= 0]
    # 储氨罐初值和终值约束
    constraint += [varsSto[0, 1] - AStoinitVal == 0,
                   varsSto[nDispatch, 1] - np.array(VirtualNH3Demand)[Day * 96 + TimeEnd - 1, 0] / 4 >= 0]
    # 氨市场约束
    constraint += [
        np.array(ActualNH3Demand)[Day * 96 + TimeBegin:Day * 96 + TimeBegin + nDispatch, 0] - varsAmmoniaMarket[:, 0] == 0]  # 合成氨需求量 = 合成氨售出量
